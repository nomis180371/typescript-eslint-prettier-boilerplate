...
const migrateData = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { type } = req.body;
    const paramsRepository = yield data_source_1.AppDataSource.getRepository(scrappingParamsEntity_1.ScrappingParams);
    try {
        if (type === 'brand') {
            ...
            for (let catalogType of catalog_json_1.default[1].catalogs) {
                const newType = scrappingParamsEntity_1.ScrappingParams.create({
                ...
                for (let childType of catalogType.catalogs) {
                    const newChildType = scrappingParamsEntity_1.ScrappingParams.create({
                    ...
                    for (let grandChildType of childType.catalogs) {
                        const newGrandChildType = scrappingParamsEntity_1.ScrappingParams.create({
            ...
            for (let size of size_json_1.default) {
                if (size.sizes.length > 0) {
                    const sizeItem = scrappingParamsEntity_1.ScrappingParams.create({
                    ...
                    for (let childSize of size.sizes) {
                        const childSizeItem = scrappingParamsEntity_1.ScrappingParams.create({
            ...
            for (let status of status_json_1.default) {
                const newStatus = scrappingParamsEntity_1.ScrappingParams.create({
            ...
            for (let color of color_json_1.default) {
                const newColor = scrappingParamsEntity_1.ScrappingParams.create({
